 ...
 *
 */
public class VendorExpServiceImpl extends BaseServiceImpl<VendorExp, String, VendorExpDao>
implements VendorExpService {
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#findForVendorExpapplFetchRelation(tw.com.skl.exp.kernel.model6.logic.enumeration.ApplStateEnum, tw.com.skl.exp.kernel.model6.bo.BigType.BigTypeCode, java.lang.String, tw.com.skl.exp.kernel.model6.bo.ProofType.ProofTypeCode, java.util.Date, java.util.Date, java.lang.Boolean)
	 */
	public List<VendorExp> findForVendorExpapplFetchRelation(ApplStateEnum applStateEnum, String payYearMonthBegin,
			String payYearMonthEnd, String vendorCompId, String invoiceNo, String applUserCode,
			String generalMgrSn, String loginUserDepCode, String deliverNo) {
		...
		if (StringUtils.isNotBlank(loginUserDepCode)) {
			...
			//defect:1361 UAT-C-UC1.6.5  登入時顯示所有「狀態=申請中」，需再加上「廠商費用.費用申請單.建檔人員.所屬單位」=Session變數"登入人員.所屬單位"」
	...
	private void verifyEntry(Entry entry){
		...
		if (!(EntryTypeCode.TYPE_2_3.getCode().equals(entry.getEntryType().getCode())||
				EntryTypeCode.TYPE_2_4.getCode().equals(entry.getEntryType().getCode())) ) {
			//丟出 C10012:借貸別錯誤
		...
		if (StringUtils.isNotBlank(entry.getSummary()) && entry.getSummary().length() >50) {
			//摘要長度不可超過50位數
			throw new ExpRuntimeException(ErrorCode.C10523, new String[]{
					MessageUtils.getAccessor().getMessage("tw_com_skl_exp_kernel_model6_bo_Entry_summary"),
					"50"
			});
		}
	}
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#addExpapplCDetail(tw.com.skl.exp.kernel.model6.bo.VendorExp, tw.com.skl.exp.kernel.model6.bo.Entry)
	 */
	public List<Entry> addExpapplCDetail(VendorExp exp, Entry expEntry, ExpapplCDetail expapplCDetail, List<Entry> withholdIncomeEntryList, String incomeId) {
		...
		if (null != exp.getExpapplC().getListType()) {
			...
			if (EntryTypeCode.TYPE_2_3.getValue().equals(expEntry.getEntryType().getValue())
					&&!CollectionUtils.isEmpty(expEntry.getAccTitle().getExpItems())
					&&"62080100".equals(expEntry.getAccTitle().getExpItems().get(0).getCode())) {
				...
				if (facade.getAccTitleService().isWithholdType(expEntry.getAccTitle())) {
					...
					//DEFECT5038_92稅檔不全62080703_ec0416 2018/3/26 start
					//當會計科目為62080703維持費用-勞務費-其他勞務費(92所得)時，則不需檢核代扣科目欄位是否為空
					if(title.getCode().equals("62080703")){
						/*
						...
						if (null == incomeUser) {
							//身分證字號與統編都查不到
							//查無則throw ExpRuntimeException顯示《請先於UC10.8.4所得人資料維護功能建立該所得人資料》訊息
							throw new ExpRuntimeException(ErrorCode.C10097);
						}
						/*
						 * 於廠商費用輸入費用明細時，
						 * 不論明細金額是否達起扣金額(依會計科目檔之設定判斷該會計科目是否須代扣)，
						 * 一律代扣61120105 -工員工資(本)代扣所得稅。expEntry.getAccTitle().getWithhold()
						 */
						/*
						 *     須檢核輸入的”業別代號”、”所得人證號”為必填，且必須存在「所得人資料」(#144)
						 *        若”所得人證號”為空白，則顯示《憑證資訊的所得人證號資料不能為空白》且不可儲存入檔
						 *        執行共用function《依所得人證號類別及所得人證號查詢所得人資料》，若查無則throw ExpRuntimeException顯示《請先於UC10.8.4所得人資料維護功能建立該所得人資料》訊息
						 * ▲       參數:
						 * ☆       “所得人證號類別”為身份證字號或廠商統編
						 * ☆       “所得人證號”
						 *        ”業別代號”存至勞務費科目的「分錄.業別代號」
						 *        ”所得人證號”存至科目「expEntry.getAccTitle().getWithhold()」的「分錄.所得人證號類別」及「分錄.所得人證號」
						 */
						...
						entryC = null;
					}else if(null == title.getWithhold()) {
					//DEFECT5038_92稅檔不全62080703_ec0416 2018/3/26 end
			...
			}else if (EntryTypeCode.TYPE_2_3.getValue().equals(expEntry.getEntryType().getValue())) {
	...
	private void generateCancelCodeByEntry(VendorExp exp, Entry entry){
		if (StringUtils.isBlank(entry.getCancelCode())) {
			...
			if (StringUtils.equals(AccTitleCode.PAYBLE_PERSONNEL.getCode(), entry.getAccTitle().getCode())
					&& StringUtils.isNotBlank(entry.getExpapplCDetail().getTicketYearMonth())) {
			...
			if (StringUtils.equals(AccTitleCode.PREPAY_COMPUTER_EQUIPMENT.getCode(), entry.getAccTitle().getCode()) ||
					StringUtils.equals(AccTitleCode.PREPAY_COMPUTER_SOFTWARE.getCode(), entry.getAccTitle().getCode()) ||
					exp.isInstallment()) {
			...
			if (StringUtils.equals(AccTitleCode.PREPAY_OTHER.getCode(), entry.getAccTitle().getCode())) {
				if (exp.isInstallment()) {
					...
					//非分期付款 10511204”應為”預付費用-其他: 產生規則(費用中分類(3)+西元年(後2碼)+月(2)+日(2)+流水號(5))
			...
			try {
				...
				//產生銷帳碼
				entry.setCancelCode(facade.getCancelCodeTypeService().generateCancelCode(
						entry.getAccTitle(), entry.getEntryType().getCode(),
						defaultTypeCode, null,
						null, exp.getVendor().getVendorCompId(),
						null, middleTypeCode,
						null, vendorContractTypeCode,
						null, null));
			} catch (Exception e) {
				exp.getExpapplC().getEntryGroup().getEntries().remove(entry);
	...
	protected void doProcessCarriedByStages(VendorExp exp, Entry entry, EntryGroup entryGroup) {
		...
		if (exp.isCarriedByStages()) {
			...
			//            AccTitle expAccTitle = facade.getAccTitleService().findByCode(entry.getAccTitle().getCode());
			//
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#calculateEntry(tw.com.skl.exp.kernel.model6.bo.VendorExp, tw.com.skl.exp.kernel.model6.bo.ExpapplCDetail)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#calculateTaxEntry(tw.com.skl.exp.kernel.model6.bo.VendorExp)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#deleteExpapplCDetail(tw.com.skl.exp.kernel.model6.bo.VendorExp, java.util.List)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doApproveApplyForm(java.util.List)
	 */
	public int doApproveApplyForm(List<String> expApplNoList, FunctionCode functionCode) {
		...
		for (ExpapplC expapplC : list) {
			...
			if (expapplC.getApplState().getCode().equals(ApplStateCode.FIRST_VERIFICATION.getCode())
					||expapplC.getApplState().getCode().equals(ApplStateCode.FIRST_VERIFICATION_REJECTED.getCode())
					||expapplC.getApplState().getCode().equals(ApplStateCode.RE_VERIFICATION_SEND.getCode())) {
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doNullifyVendorExp(java.util.List)
	 */
	public void doNullifyVendorExp(List<String> applyFormNoList, FunctionCode functionCode) {
		if (CollectionUtils.isEmpty(applyFormNoList)) {
			String[] params = {
					Messages.getString("applicationResources", "tw_com_skl_exp_kernel_model6_bo_ExpapplC_expApplNo", null)
			};
		...
		if (CollectionUtils.isEmpty(expapplCList)) {
			String[] params = {
					Messages.getString("applicationResources", "tw_com_skl_exp_kernel_model6_bo_ExpapplC", null)+"List"
			};
			throw new ExpRuntimeException(ErrorCode.A20002, params);
		}
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doReapplyApplyForm(tw.com.skl.exp.kernel.model6.bo.VendorExp)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doReturnApplyForm(tw.com.skl.exp.kernel.model6.bo.VendorExp)
	 */
	public void doReturnApplyForm(VendorExp exp, FunctionCode functionCode, String returnCauseCode, String returnStatement) throws ExpException, ExpRuntimeException {
		if (null == exp || null == functionCode ||
				StringUtils.isBlank(returnStatement)|| StringUtils.isBlank(returnCauseCode)) {
			List<String> errorStringList = new ArrayList<String>();
			if (null == exp) {
				//廠商費用
				errorStringList.add(MessageUtils.getAccessor().getMessage("tw_com_skl_exp_kernel_model6_bo_VendorExp"));
	...
	private void checkProofAdd(VendorExp exp){
		if (null == exp
				|| null == exp.getExpapplC()
				||StringUtils.isBlank(exp.getExpapplC().getProofAdd())) {
			return;
		}
		...
		if (!StringUtils.equals(exp.getVendor().getVendorCompId(), vendorExp.getVendor().getVendorCompId())) {
			...
			throw new ExpRuntimeException(ErrorCode.C10342, new String[]{
					MessageUtils.getAccessor().getMessage("tw_com_skl_exp_kernel_model6_bo_Vendor_vendorCompId3"),
					MessageUtils.getAccessor().getMessage("tw_com_skl_exp_kernel_model6_bo_Vendor_vendorCompId3")});
		}
	...
	public void doSaveExp(FunctionCode functionCode, VendorExp exp) {
		...
		if(null != exp){
			...
			if(null != exp.getExpapplC()){
				checkProofAdd(exp);
				if (null == exp.getExpapplC().getEntryGroup() ||
						CollectionUtils.isEmpty(exp.getExpapplC().getEntryGroup().getEntries())) {
					List<String> errorStringList = new ArrayList<String>();
					if (null == exp.getExpapplC().getEntryGroup()) {
					...
					if (exp.getExpapplC().isNeedTaxRemit() == false
							&& null != exp.getExpapplC().getEntryGroup()
							&& !CollectionUtils.isEmpty(exp.getExpapplC().getEntryGroup().getEntries())) {
						for (Entry entry : exp.getExpapplC().getEntryGroup().getEntries()) {
							if (StringUtils.equals("63300205", entry.getAccTitle().getCode())
									|| StringUtils.equals("63300406", entry.getAccTitle().getCode())) {
				...
				if(null == exp.getExpapplC().getListType() ||
						StringUtils.isEmpty(exp.getExpapplC().getListType().getCode())){
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doUpdateExp(tw.com.skl.exp.kernel.model6.bo.Function.FunctionCode, tw.com.skl.exp.kernel.model6.bo.VendorExp, java.util.List)
	 */
	...
	public List<VendorExpApproveDto> findForApproveDto(MiddleType middleType, String payYearMonth, String invoiceNo,
			String vendorCompId, String userId, String expApplNo, String firstVerifyUserId, String sendItemNo,
			Boolean isTemporaryPayment, ApplStateCode[] states) {

		return getDao().findForApproveDto(
				middleType, payYearMonth,
				invoiceNo, vendorCompId,
				userId, expApplNo,
				firstVerifyUserId, sendItemNo,
				isTemporaryPayment, states);
	}

	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#findForApprove(tw.com.skl.exp.kernel.model6.bo.MiddleType, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.String, java.lang.Boolean, java.util.Calendar, tw.com.skl.exp.kernel.model6.bo.ApplState.ApplStateCode[])
	 */
	public List<VendorExp> findForApprove(MiddleType middleType, String payYearMonth, String invoiceNo,
			String vendorCompId, String userId, String expApplNo, String firstVerifyUserId, Boolean isTemporaryPayment,
			Calendar returnDate, ApplStateCode[] states) {
		...
		queryString.append(" order by vendorExp.expapplC.middleType.code" +
		", vendorExp.generalMgrSn");
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#isInvoiceAmtEqualsRealityAmt(tw.com.skl.exp.kernel.model6.bo.VendorExp)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#updateExpapplCDetail(tw.com.skl.exp.kernel.model6.bo.VendorExp, java.lang.Integer, tw.com.skl.exp.kernel.model6.bo.ExpapplCDetail)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#verifyExp(tw.com.skl.exp.kernel.model6.bo.VendorExp, tw.com.skl.exp.kernel.model6.bo.ExpapplCDetail)
	 */
	public void verifyExp(VendorExp exp, Entry entry) {
		...
		if ("62100023".equals(entry.getAccTitle().getCode())) {
			 ...
			 * (系統產生廠商付款明細，實際付款對象為「代墊廠商統編」，非頁面上輸入之「廠商統一編號」，代墊付款廠商僅影響付款明細，若有稅額資料以UC1.5.9「廠商統編」欄位產生進項稅或所得稅。)
			 */
		...
		//檢查廠商是否存在
		if (null == exp.getVendor() ||
				!facade.getVendorService().checkVendorCompId(exp.getVendor().getVendorCompId())) {
			throw new ExpRuntimeException(ErrorCode.A10007, new String[]{MessageUtils.getAccessor().getMessage("tw_com_skl_exp_kernel_model6_bo_Vendor_vendorCompId")});
		}
		...
		// TODO 輸入借方明細時，若科目代號為"61440123(餐費)、62100023(交際費)"時，須檢核成本單位代號之"餐費、交際費"預算項目不可超支， 若申請金額超過可申請限額，則顯示已超支且不可儲存送出申請，若有輸入”專案代號”時，則不在此限。

	}

	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#calculateVerifyUserInputVendorExpCount(java.lang.String, java.lang.String)
	 */
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#findByDeliverDaylistNoFetchVendor(java.lang.String)
	 */
	public List<VendorExp> findByDeliverDaylistNoFetchVendor(String deliverDaylistNo) throws ExpException {
		...
		List<VendorExp> vendorExpList = this.findForVendorExpapplFetchRelation(
				null, null, null, null, null, null, null, null, deliverDaylistNo);
	...
	public List<VendorExpDaylistDetailDto> findVendorExpDaylistDetailDtoByDeliverDaylistNo(String deliverDaylistNo)
	throws ExpException {
		if (StringUtils.isBlank(deliverDaylistNo)) {
			String[] params = {
					Messages.getString("applicationResources", "tw_com_skl_exp_kernel_model6_bo_DeliverDaylist_deliverNo", null)
			};
			//找不到分錄群組
	...
	public void checkInvoice(String invoiceNo, Calendar invoiceDate, String expApplNo) {
		...
		queryString.append(" where");
		queryString.append("  vendorExp.expapplC.invoiceNo =:invoiceNo "
				+ " and vendorExp.expapplC.invoiceDate >=:invoiceDateStart"
				+ " and vendorExp.expapplC.invoiceDate <:invoiceDateEnd"
				+ " and vendorExp.expapplC.applState.code <>:applStateCode");
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#findApplyAmtByVendorContractCode(java.lang.String, tw.com.skl.exp.kernel.model6.bo.ApplState.ApplStateCode)
	 */
	public BigDecimal findApplyAmtByVendorContractCode(String vendorContractCode, ApplStateCode applStateCode)
	throws ExpException, ExpRuntimeException {
	...
	private List<Entry> generateWithholdIncome(ExpapplC expapplC, Entry entry, List<Entry> entryList) {
		...
		if (expapplC.isWithholdIncome()) {
			...
			if (expapplC.isHadAllowanceSlip()) {
				...
				//                //金額大於0 則產生貸方折讓分錄(費用)
				//                if (BigDecimal.ZERO.compareTo(expapplC.getAllowanceNoneTaxAmt()) < 0) {
	...
	public List<VendorExpMonthCloseDto> findForVendorExpMonthClose(String payYearMonth, Calendar expectRemitDate) {
		...
		querySql.append("select vexp from VendorExp vexp" +
				" join vexp.expapplC eac" +
				" join eac.middleType mt" +
				" join mt.bigType bt" +
				" where eac.inCloseTemporaryPayment='1' and eac.applState.code='70'" +
				" and bt.code='01'" +
		" and vexp.payYearMonth=:payYM and eac.expectRemitDate=:erDate");
		Map<String,Object> params = new HashMap<String, Object>();
	...
	/**
	 * <p>產生送匯批次記錄，共用方法 for C 5.1.1廠商月結 & C 5.1.2廠商內結臨付</P>
	 * @param sysDate 系統時間
	...
	/* (non-Javadoc)
	 * @see tw.com.skl.exp.kernel.model6.logic.VendorExpService#doMonthlyClose(java.util.List)
	 */
	public void doMonthlyClose(List<String> subpoenaNoList, Calendar accountDate)throws ExpException {
		...
		for(String subpoenaNo : subpoenaNoList){
			...
			for(ExpapplC eac : eacList){
				...
				sleepCounter++;
				dataMap = doProcessExpapplCForMonthClose1(eac, tempDebitBigEn, tempCreditRemitBigEn,
						tempCreditCheckBigEn, sysDate, allToSaveEntries, allToSaveTpds);
		...
		if(org.apache.commons.collections.CollectionUtils.isNotEmpty(eacNeedUpdateList)){
			...
			for (ExpapplC eac: eacNeedUpdateList){
				facade.getPaymentDetailService().generatePaymentDetail(eac, paymentBatch, sysDate);//TODO: update
	 ...
	 * <p>C 5.1.1</p>
	 * <p>廠商月結</p>
	 * <ul><li>只要產生一筆日結單代傳票，而且不用和原有的費用申請單設關聯!!</li></ul>	 *
	 * @param eac 申請單
	 * @param tempDebitBigEn 暫存的借方大分錄
	 * @param tempCreditRemitBigEn 暫存的貸方待匯大分錄
	 * @param tempCreditCheckBigEn 暫存的貸方待開大分錄
	 * @param sysDate 系統時間
	 * @param allToSaveEntries 待儲存的分錄
	 * @param allToSaveTpdList 待儲存的過渡付款明細
	 * @return
	 * @throws ExpException
	 */
	private Map<String, Object> doProcessExpapplCForMonthClose1(
			ExpapplC eac,
			BigEntry tempDebitBigEn,
			BigEntry tempCreditRemitBigEn,
			BigEntry tempCreditCheckBigEn,
			Calendar sysDate,
			List<ToUpdateEntry> allToSaveEntries,
			List<ToUpdateTransitPaymentDetail> allToSaveTpds
			)throws ExpException {
		...
		//查出廠商資料
		Vendor vendor = findByVendorExpUseExpapplC(eac); //modified by Jackson 2010/2/10
			...
			if(AccTitleCode.PAYBLE_LEDGER.getCode().equals(en.getAccTitle().getCode())){
				...
				if(PaymentTypeCode.C_CHECK.getCode().equals(eac.getPaymentType().getCode())){
					...
					if(tempCreditCheckBigEn == null
							|| tempCreditCheckBigEn.getBigEntryAmt().compareTo(BigDecimal.ZERO)==0){
						...
						try{tempCreditCheckBigEn.setSummary(acct.getName());}catch(Exception e){}
						//大分錄裡面只要有一組銷帳碼就好!! #168
				...
				/*付款方式為匯款時*/
					...
					if(tempCreditRemitBigEn == null
							|| tempCreditRemitBigEn.getBigEntryAmt().compareTo(BigDecimal.ZERO)==0){
						...
						try{tempCreditRemitBigEn.setSummary(acct.getName());}catch(Exception e){}
						if(StringUtils.isBlank(tempCreditRemitBigEn.getCancelCode())){ //大分錄裡面只要有一組銷帳碼就好!! #168
							String cancelCode = genBigEntryCancelCode(en, acct);
...
//		}
	...
	/**
	 * <p>C 5.1.1 廠商月結 & C 5.1.2 廠商內結臨付</p>
	 * <ul><li>將分錄資料過到大分錄，但未建立與日結單代傳票的關聯</li></ul>
	 * @param bigEn 大分錄
	 * @param en 分錄
	 ...
	 * @return 過渡付款明細List
	 */
	private List<TransitPaymentDetail> findAndCheckTransitPaymentDetails(
			ExpapplC eac, Entry en) {
	 ...
	 * @param eac 申請單
	 * @param en 分錄
	 * @param countAmt 過渡付款明細總金額
	 */
	private void checkTransitPaymentDetailAmt(ExpapplC eac, Entry en,
			BigDecimal countAmt) {
	...
	private String genBigEntryCancelCode(Entry en, AccTitle acct){
		SNGenerator cancelCodeGenerator = facade.getCancelCodeTypeService().getCancelCodeGenerator(
				acct, en.getEntryType().getValue());
		Map<String, String> params = new HashMap<String, String>();
	...
	/**
	 * <p>C 5.1.1 廠商月結</p>
	 * <ul><li>產生日結單代傳票</li></ul>
	 * @param btCode 費用大分類代號
	 * @return genSubpoena 日結單代傳票
	 */
	private Subpoena genSubpoena(String btCode, Calendar subpoenaDate){
		/*
		 * 傳票號碼原則，傳票號碼編碼方式：一碼"S" + 二碼識別碼（費用大分類代碼）
		 * +1 碼年（西元年最末碼，即0~9） + 1 碼月（1~9 代表1~9 月，A、B、C 代表10、11、12 月）
		 * + 2 碼日 + 3 碼流水號。(SubpoenaNoGenerator)
		 */
	...
	/**
	 * <p>C 5.1.1 廠商月結 & C 5.1.2 廠商內結臨付</p>
	 * <ul><li>以大分錄資料產生新的分錄群組，以及分錄資料
	 * 並分回新產生的日結單代傳票</li></ul>
	 * @param bigEnList 大分錄資料
	 * @param subpoena 新產生的日結單代傳票
	...
	public List<VendorExp> findForInCloseTempPay(String payYearMonth, String invoiceNo, boolean inCloseTempPay,
			boolean tempNoPay) {
		if(payYearMonth==null){
			throw new ExpRuntimeException(ErrorCode.A10007,
					new String[]{Messages.getString("applicationResources", "tw_com_skl_exp_kernel_model6_bo_VendorExp_payYearMonth", null)});
		}
		StringBuffer querySql = new StringBuffer();
		querySql.append("select vExp from VendorExp vExp join vExp.expapplC eac join eac.middleType mt join mt.bigType bt join eac.applState aplState" +
		" where vExp.payYearMonth=:pym and aplState.code='70' and bt.code='01'");
		Map<String,Object> params = new HashMap<String, Object>();
	...
	private List<VendorExp> findVendorExpUseExpApplNoList(List<String> expApplNoList){
		if(org.apache.commons.collections.CollectionUtils.isEmpty(expApplNoList))return null;
	...
	public void doConfirmInCloseTemporaryPayment(List<String> expApplNoList, Calendar expectRemitDate) throws ExpException{
		...
		if(CollectionUtils.isEmpty(eac2Save))return;
		for(ExpapplC eac2 : eac2Save){
			dataMap = doProcessExpapplCForMonthClose(eac2, newSubp, paymentBatch, tempDebitBigEn, tempCreditRemitBigEn, tempCreditCheckBigEn,
					sysDate, expectRemitDate);
			tempDebitBigEn = (BigEntry)dataMap.get("tempDebitBigEn");
			tempCreditRemitBigEn = (BigEntry)dataMap.get("tempCreditRemitBigEn");
			tempCreditCheckBigEn = (BigEntry)dataMap.get("tempCreditCheckBigEn");
		}
	...
	public void caculateIncomeAmt(VendorExp vendorExp) {
		...
		if (vendorExp.getExpapplC().isWithholdIncome()) {
			...
			if (null != vendorExp.getExpapplC().getInvoiceAmt() &&
					vendorExp.getExpapplC().getInvoiceAmt().compareTo(BigDecimal.ZERO)>0) {
	...
	public List<VendorExpInCloseTempPayDto> findForInCloseTempPay(String payYearMonth, String invoiceNo,
			Boolean inCloseTemporaryPay, Boolean temporaryNoPayment) {
	 ...
	 * <p>確認內結臨付</p>
	 * <ul><li>只要產生一筆日結單代傳票，而且不用和原有的費用申請單設關聯!!</li></ul>
	 * @param newSubp 代回傳的日結單代傳票
	 * @param paymentBatch 送匯批次記錄
	 * @param tempDebitBigEn 暫存的借方大分錄
	 * @param tempCreditRemitBigEn 暫存的貸方待匯大分錄
	 * @param tempCreditCheckBigEn 暫存的貸方待開大分錄
	 * @param sysDate 系統時間
	 * @param expectRemitDate 預計付款日期，確認內結臨付才會傳
	 * @param eacList 費用申請單集合
	 * @return Map 資料群
	 * @throws ExpException
	 */
	private Map<String, Object> doProcessExpapplCForMonthClose(ExpapplC eac, Subpoena newSubp, PaymentBatch paymentBatch
			, BigEntry tempDebitBigEn, BigEntry tempCreditRemitBigEn, BigEntry tempCreditCheckBigEn,
			Calendar sysDate, Calendar expectRemitDate)throws ExpException {
			...
			if(AccTitleCode.PAYBLE_LEDGER.getCode().equals(en.getAccTitle().getCode())){
				...
				if(PaymentTypeCode.C_CHECK.getCode().equals(eac.getPaymentType().getCode())){
					if(tempCreditCheckBigEn == null){
						...
						// 2009/12/24, 摘要放會計科目中文IR#1625
						try{tempCreditCheckBigEn.setSummary(acct.getName());}catch(Exception e){}
						if(StringUtils.isBlank(tempCreditCheckBigEn.getCancelCode())){ //大分錄裡面只要有一組銷帳碼就好!! #168
							String cancelCode = genBigEntryCancelCode(en, acct);
							tempCreditCheckBigEn.setCancelCode(cancelCode);
